  ...
  <div class="container">
    ...
    <form>
      <div class="mb-3">
        ...
        <input type="text" class="form-control" id="exampleInput1" v-model="nombre"/>
      </div>
      <div class="mb-3">
        <label for="exampleFormControlTextarea1" class="form-label">Opiniones</label>
        <textarea class="form-control" id="exampleFormControlTextarea1" rows="3" v-model="opinion"></textarea>
      </div>
      <button type="button" class="btn btn-secondary" @click="regresar"> Regresar </button>
      <button
        type="button" id="bt" class="btn btn-info" @click="actualizarOpinion(id - 1)"> Guardar </button>
    </form>
  </div>
</template>
